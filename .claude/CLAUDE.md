...
- **前端**: React Router（`apps/web`）+ 共享 shadcn/ui（`packages/ui`）；**本作业页面由 Hono 直出 HTML**（`apps/server/src/page.ts`），未用 web 应用
...
| --- | --- | --- | --- |
| GET | `/` | Basic Auth | 表单页面 |
| POST | `/api/github` | Basic Auth | token 取 GitHub 账户并「增」一条 |
| GET | `/api/users` | Basic Auth | 列出已保存账户（createdAt 倒序） |
| DELETE | `/api/users/:id` | Basic Auth | 「删」一条记录 |
| GET | `/health` | 放行 | 健康检查 |
